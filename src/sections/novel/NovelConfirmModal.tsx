...
interface NovelConfirmModalProps {
  open: boolean;
...
export function NovelConfirmModal({
  open,
  action,
  numSelected,
  onClose,
  onConfirm
}: NovelConfirmModalProps) {
  ...
  const getActionTitle = () => {
    switch (action) {
      case 'disable':
        return "Disable Selected Novel";
      case 'enable':
        return "Enable Selected Novel";
      case 'resetImages':
        return "Reset Images for Selected Novel";
      default:
    ...
    >
      ...
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          Are you sure you want to {getActionText()} <b>{numSelected}</b> selected novel{numSelected > 1 ? 's' : ''}?
